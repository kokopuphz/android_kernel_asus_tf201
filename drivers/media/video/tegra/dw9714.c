...
// It is not necessary to have POS_LOW and POS_HIGH is kernel code.
// reason: the effective ranage of a focuser depends on lens too.
//         upper layer can decide the range to call into kernel.
// still keep them here in the code and use max range.
//
#define POS_LOW       0
...
static int dw9714_write(struct i2c_client *client, u16 value)
{
	...
	if (!client->adapter)
 	...
 	data[1] = value & 0xff;
#else
        // when there is no linear slope control
        //
        // dw9714 directly write. Need to check vendor to confirm the bit shift
        // below is indeed correct. It is known working for dw9712 module.
        // dw9712/(dw9714?) direct write fashion (with linear slip control set to 0x0)
