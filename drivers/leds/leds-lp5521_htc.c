...
#define LED_DEBUG 0
	...
	#define MF_DEBUG(x...) printk(KERN_INFO x)
#else
	#define D(x...)
	#define I(x...)
	#define MF_DEBUG
...
static int __init lp5521_led_tag(char *tag)
{
	...
	if (strlen(tag))
		stu = strict_strtoul(tag, 16, &lp5521_led_tag_status);
	if (stu < 0)
		pr_warn("[EPRJ][LP5521] Can't get LED TAG status.\n");
...
static void lp5521_led_early_suspend(struct early_suspend *handler)
{
	...
	I("[SUSPEND] lp5521_led_early_suspend +++\n");
	suspend_mode = 1;
	if( backlight_mode == 1 )
		lp5521_backlight_off(client);
	else if ( backlight_mode == 2 )
...
{
	I("[RESUME] lp5521_led_late_resume +++\n");
	suspend_mode = 0;
	I("[RESUME] lp5521_led_late_resume ---\n");
}
