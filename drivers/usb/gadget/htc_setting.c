...
#define USB_INFO(x...) pr_info(x);
#define USB_ERR(x...) pr_err(x);
...
static void send_usb_connect_notify(struct work_struct *w)
{
	...
	if (!udc) {
		pr_err("EPRJ: ERROR: send_usb_connect_notify: !udc \n");
		return;
	}
		...
		notifier_link) {
			if (notifier->func != NULL) {
				...
				if (udc->connect_type == CONNECT_TYPE_UNKNOWN)
					notifier->func(CONNECT_TYPE_CDP);
				else
...
static void update_wake_lock(int status)
{
	if ((status == CONNECT_TYPE_CDP || status == CONNECT_TYPE_UNKNOWN) && board_mfg_mode() != BOARD_MFG_MODE_OFFMODE_CHARGING) {
...
#define DELAY_FOR_CHECK_CHG msecs_to_jiffies(250)
static void charger_detect(struct tegra_udc *udc)
{
	u32 portsc;
	u32 ret;
...
#endif
	{
		...
		udc->connect_type = CONNECT_TYPE_CDP; //CONNECT_TYPE_UNKNOWN;
...
static void check_charger(struct work_struct *w)
{
	...
	/* EPRJ: Unknown charger or CDP mode */
	if (vbus_enabled(udc) &&
...
static void ac_detect_expired_work(struct work_struct *w)
{
	...
	if (udc->connect_type == CONNECT_TYPE_CDP || udc->ac_detect_count >= 3)
		return;
#if defined(CONFIG_CABLE_DETECT_ACCESSORY)
