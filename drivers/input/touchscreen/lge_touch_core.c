...
static void touch_fw_upgrade_func(struct work_struct *work_fw_upgrade)
{
	...
	TOUCH_INFO_MSG("config_id :%d fw_image_config_id:%d\n",
		       config_id,fw_image_config_id);

	if (likely(touch_debug_mask & (DEBUG_FW_UPGRADE | DEBUG_BASE_INFO)))
		TOUCH_INFO_MSG("config_id_ver[ic:%d/fw:%d] config_id[ic:%s/fw:%s] config_id_int[ic:%d/fw:%d]\n",
	...
	if((config_id_ver == fw_image_config_id_ver) && (config_id < fw_image_config_id))
	    b_need_upgrade = 1;
	else if ((config_id_ver != fw_image_config_id_ver)&& (fw_image_config_id_ver != CONFIG_ID_NONE )) 
	    b_need_upgrade = 1;
...
static struct platform_driver eprj_lgtouch_driver = {
	.driver	= {
		.name = LGE_TOUCH_NAME,
	},
};

static struct platform_device *eprj_lgtouch_pdev;
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* EternityProject: Register the driver */
	ret = platform_driver_register(&eprj_lgtouch_driver);
	if (ret) {
		TOUCH_ERR_MSG("[EPRJ] platform_driver_register have failed\n");
		goto err_lge_touch_platform_driver_register;
	}

	/* EternityProject: Register the device to activate */
	eprj_lgtouch_pdev = platform_device_register_simple(
				eprj_lgtouch_driver.driver.name, 0, NULL, 0);
	if (IS_ERR(eprj_lgtouch_pdev)) {
		ret = PTR_ERR(eprj_lgtouch_pdev);
		/* EPRJ: Avoid memory leaking */
		platform_driver_unregister(&eprj_lgtouch_driver);
		goto err_lge_touch_platform_driver_register;
	}
...
static int touch_remove(struct i2c_client *client)
{
	...
	kobject_del(&ts->lge_touch_kobj);
	platform_device_unregister(eprj_lgtouch_pdev);
	platform_driver_unregister(&eprj_lgtouch_driver);
