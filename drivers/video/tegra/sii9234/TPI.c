...
extern int TPI_SLAVE_ADDR;
extern int HDMI_SLAVE_ADDR;
extern int CBUS_SLAVE_ADDR;
extern int TPID_SLAVE_ADDR;
...
enum tegra_connect_type gStatusMHL = CONNECT_TYPE_UNKNOWN;
...
bool TPI_Init(void)
{
	...
	TxHW_Reset();
	PR_DISP_DEBUG("Drv: SiiMhlTxChipInitialize: %02X44\n", (int)I2C_ReadByte(TPI_SLAVE_ADDR, 0x03));
	if (!ProductID_Read())
		return FALSE;
...
static void WriteInitialRegisterValues(void)
{
	...
	I2C_WriteByte(TPID_SLAVE_ADDR, 0x3D, 0x3F);
	...
	if (!sii9234_get_ci2ca())
		...
		I2C_WriteByte(TPI_SLAVE_ADDR, 0x91, 0xAD);
...
void CbusWakeUpPulseGenerator(void)
{
	...
	regval &= 0x3F;
	I2C_WriteByte(TPI_SLAVE_ADDR, 0x96, regval);
	DelayMS(T_SRC_WAKE_PULSE_WIDTH_2 - 1);
...
static void SwitchToD3(void)
{
	if (POWER_STATE_D3 != fwPowerState) {
		...
		CLR_BIT(TPID_SLAVE_ADDR, 0x3D, 0);
