...
#if defined(MACH_X3_REV_B) || defined(MACH_X3_REV_C)

#define APDS990x_PS_DETECTION_THRESHOLD						450     
#define APDS990x_PS_HSYTERESIS_THRESHOLD					350     
#else
...
#define APDS990x_PS_DEFAULT_PPCOUNT_REV_B					0x07			
#define APDS990x_PS_DEFAULT_CROSSTALK_VAL_REV_B				150

#define APDS990x_PS_DEFAULT_PPCOUNT_REV_D					0x03			
#define APDS990x_PS_DEFAULT_CROSSTALK_VAL_REV_D				650


//Calculation parameters
#define APDS990x_PS_DETECTION_THRESHOLD_BASE				250
#define APDS990x_PS_HSYTERESIS_THRESHOLD_SUBTRACT_VAL		80
...
static int PS_DEFAULT_PPCOUNT = APDS990x_PS_DEFAULT_PPCOUNT_REV_B;
static int PS_DEFAULT_CROSSTALK_VAL = APDS990x_PS_DEFAULT_CROSSTALK_VAL_REV_B;

static int GA = APDS990x_BLACK_ALS_DEFAULT_GA; 		/* 2.485 --- 0.48 without glass window ..old: 271*/
static int COE_B = APDS990x_BLACK_ALS_DEFAULT_COE_B;		/* 1.973 --- 2.23 without glass window ..old: 231*/
static int COE_C = APDS990x_BLACK_ALS_DEFAULT_COE_C;		/* 0.694 --- 0.70 without glass window .. old: 34*/
static int COE_D = APDS990x_BLACK_ALS_DEFAULT_COE_D;		/* 1.322 --- 1.42 without glass window .. old: 65*/
...
static int apds990x_set_enable(struct i2c_client *client, int enable)
{
	...
	if(DEBUG_DEV_DEBOUNCE & debug_mask) 
	{
		...
		ppc = i2c_smbus_read_byte_data(client, CMD_BYTE|APDS990x_PPCOUNT_REG);		
...
RE_CALIBRATION:
	...
	if (white_check_flag == 0)
		apds990x_set_ppcount(client, 0x07);		
	else
...
static void apds990x_work_handler(struct work_struct *work)
{
	...
	if(DEBUG_DEV_DEBOUNCE & debug_mask)
	{
		...
		DEBUG_MSG("[apds990x] cdata = %d, irdata = %d, pdata = %d\n", cdata,irdata,pdata);
		DEBUG_MSG("[apds990x] ailt = %d, aiht = %d\n", ailt,aiht);
		DEBUG_MSG("[apds990x] pilt = %d, piht = %d\n", pilt,piht);
				   ...
				   const struct i2c_device_id *id)
{
	...
	{
		dev_err(&client->dev,
				"%s: failed to read apds990x platform data\n",
				__func__);
		return -ENODEV;
	}
	...
	data->is_white = 0;			
        ...
        printk("%s : get cross-talk = %d , ppcount_val = %d \n",__func__,cross_talk_val, ppcount_val);
	...
	printk("[prox] apds990x_probe enable = %x\n", data->enable);
	...
	if (err < 0) {
		dev_err(&client->dev,
				"%s: Could not allocate APDS990x_INT !\n",
				__func__);
		goto exit_kfree;
	}
	...
	dev_info(&client->dev, "%s: interrupt is hooked\n", __func__);

	/* Initialize the APDS990x chip */
	err = apds990x_init_client(client);
	if (err < 0) {
		dev_err(&client->dev, "%s: apds990x_init_client() failed\n",
				__func__);
		goto exit_free_irq;
	}
	...
	if (!data->input_dev_als) {
		err = -ENOMEM;
		dev_err(&client->dev,
				"%s: Failed to allocate input device als\n",
				__func__);
		goto exit_free_irq;
	}
	...
	if (!data->input_dev_ps) {
		err = -ENOMEM;
		dev_err(&client->dev,
				"%s: Failed to allocate input device ps\n",
				__func__);
	...
	if (err < 0) {
		dev_err(&client->dev,
				"%s: Unable to register input device als: %s\n",
	...
	if (err < 0) {
		dev_err(&client->dev,
				"%s: Unable to register input device ps: %s\n",
	...
	printk("%s prior data->enable_als_sensor = %d LINE[%d]\n",__func__,data->enable_als_sensor,__LINE__);
