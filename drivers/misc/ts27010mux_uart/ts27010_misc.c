...
static u8 s_hexdump_buf[TS0710MUX_SEND_BUF_SIZE * 3 + 2 + 64 + 1];
	...
	const char *function, u32 line, const u8 *buf, u32 len)
{
	...
	printk(KERN_WARNING "MUX_UART[%s:%d]: ", function, line);
...
void mux_uart_hexdump(int level, const char *title, const char *function,
	u32 line, const u8 *buf, u32 len)
{
	...
	if (buf == NULL) {
		...
		snprintf(s_hexdump_buf, 63,
			"MUX_UART: hexdump[%s-%d]: ", title, len);
		s_hexdump_buf[63] = 0;
	...
	int count, int *eof, void *data)
{
	...
	ts27010_tty_uart_dump_io();
	ts27010_ldisc_uart_drv_stat();
	ts27010_mux_uart_mux_stat();
...
#define PROC_BUF_LEN 16
	...
	if (copy_from_user(k_buf, buff, count)) {
		...
		g_nStatUARTDrvIO = (int)para;
		mux_print(MSG_INFO, "stat mux=%d", g_nStatUARTDrvIO);
		if (g_nStatUARTDrvIO) {
			ts27010_tty_uart_dump_io_clear();
			ts27010_ldisc_uart_drv_stat_clear();
...
#define PROC_BUF_LEN 16
	...
	if (copy_from_user(k_buf, buff, count)) {
		...
		if (para <= MSG_NONE) {
			g_mux_uart_print_level = (int)para;
		...
		g_mux_uart_dump_user_data = para ? 1 : 0;
	...
	PROC_READ read, PROC_WRITE write)
{
	...
	if (proc_mux == NULL) {
		printk(KERN_WARNING "create muxuart proc file failed!\n");
	} else {
...
struct proc_dir_entry *ts27010_uart_proc_alloc(void)
{
	return ts27010_create_proc("muxuart", mux_proc_read, mux_proc_write);
}

#ifdef PROC_DEBUG_MUX_STAT
struct proc_dir_entry *ts27010_uart_proc_stat_alloc(void)
{
	return ts27010_create_proc("muxuart_stat",
...
{
	queue_work(g_mux_uart_queue, &timer_para->m_send_work);
}
			...
			u8 *data, u32 len)
{
	...
	if ((short_pkt->h.length.ea) == 0) {
		...
		long_pkt->data[data_len] = ts0710_uart_crc_data((u8 *)long_pkt,
			(LONG_CRC_CHECK + data_len));
	} else {
		data_len = short_pkt->h.length.len;
		short_pkt->data[data_len] = ts0710_uart_crc_data((u8 *)short_pkt,
