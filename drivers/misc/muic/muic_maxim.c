...
extern void write_muic_data(void);
extern s32 read_muic_data(void);
...
//			set_max14526_ap_usb_mode(); /* Should we? Dunno.... */
			/* USB Host or MHL? MHL happens only on VBUS ON, so it's USB Host. */
			pr_info("[EPRJ-MUIC_MAXIM] USB Host configuration\n");
#if 0
			usif_switch_ctrl(USIF_AP);
			dp3t_switch_ctrl(DP3T_CP_UART);

			/* Enables USB Path (0x03 = 0x23) */
//			muic_i2c_write_byte(SW_CONTROL, COMP2_TO_HZ | COMN1_TO_C1COMP);
			muic_i2c_write_byte(SW_CONTROL, COMP2_TO_DP2 | COMN1_TO_DN1);
			muic_mdelay(1);
...
//			value = read_muic_data();
//			check_cable_ID = cable_id_check(int_stat_value);
#endif
		} else {
...
s32 muic_max14526_detect_accessory(s32 upon_irq)
{
	...
	case MUIC_CP_USB :
		if ((int_stat_value & IDNO) == IDNO_0000) {
			pr_info("[EPRJ-MUIC-MAXIM] Accessory detection: USB Host Mode.\n");
			return 101;
		}
