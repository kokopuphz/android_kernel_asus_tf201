...
#define ETERNITYPROJECT_IDNO_DEBUG
#define ETERNITYPROJECT_USBHOST

#ifdef ETERNITYPROJECT_USBHOST
extern bool is_eprj_usbh(void);
extern void usb_host_status_notifier_func(bool isEnable);
#endif

void eprj_x3_usbhost_switch(bool enb)
{
	if (enb) {
		retain_mode = RETAIN_AP_USB;
		muic_proc_set_ap_usb();
...
{
#ifdef ETERNITYPROJECT_IDNO_DEBUG
	int chgval = 0;

	chgval = value & V_VBUS;

	pr_info("[EPRJ-MUIC] Cable ID is 0x%x\n", (value & IDNO));
#endif
#ifdef ETERNITYPROJECT_USBHOST
	value = gpio_get_value(TEGRA_GPIO_PS2);

	pr_info("[EPRJ-MUIC] [USBHOST] TEGRA_GPIO_PS2 is %d.\n", value);
#endif
...
void check_charging_mode(void)
{
	...
	if (value & V_VBUS) {
		if ((value & IDNO) == IDNO_0010 ||
			...
			|| retain_mode == RETAIN_AP_USB)
		{
			pr_info("[EPRJ-MUIC] %s: Selecting CHARGING_FACTORY.\n", __func__);
			charging_mode = CHARGING_FACTORY;
		}
		else if (value & CHGDET)
		{
			pr_info("[EPRJ-MUIC] %s: Selecting CHARGING_LG_TA.\n", __func__);
			charging_mode = CHARGING_LG_TA;
		}
		else
		{
			pr_info("[EPRJ-MUIC] %s: Selecting CHARGING_USB\n", __func__);
			charging_mode = CHARGING_USB;
		}
	} else {
//		if ((value & IDNO) == IDNO_0000) {
//			pr_info("[EPRJ-MUIC] %s: USB Host mode or MHL detected.\n", __func__);
//			charging_mode = CHARGING_EPRJUSBH;
//			charging_mode = CHARGING_FACTORY;
//		} else
		{
			pr_info("[EPRJ-MUIC] %s: Selecting CHARGING_NONE\n", __func__);
			charging_mode = CHARGING_FACTORY;
		}
	}
...
//	extern void usb_host_status_notifier_func(bool isEnable);
	...
	switch (mode) {
		case CHARGING_USB:
		case CHARGING_MHL:		//                                                      
			pr_info("[EPRJ-MUIC] %s: Get USB/MHL power supply\n", __func__);
		...
		case CHARGING_EPRJUSBH:
			pr_info("[EPRJ-MUIC] %s: Get EternityProject USBH power supply\n", __func__);
			psy = power_supply_get_by_name("usb");
			value.intval = POWER_SUPPLY_TYPE_USB;
			break;
		...
		case CHARGING_FACTORY:
			pr_info("[EPRJ-MUIC] %s: Get factory power supply\n", __func__);
...
//			usb_host_status_notifier_func(1);
		...
		case CHARGING_NONE:
			pr_info("[EPRJ-MUIC] %s: Get NONE(BATTERY) power supply\n", __func__);
			psy = power_supply_get_by_name("battery");
			value.intval = POWER_SUPPLY_TYPE_BATTERY;
			break;

		default:
			pr_info("[EPRJ-MUIC] %s: Get BATTERY power supply\n", __func__);
			value.intval = POWER_SUPPLY_TYPE_BATTERY;
			break;
	}
...
static void muic_work_inside_operation (void) {
	...
	if (retain_mode == RETAIN_NO) {
		...
		if (ret == 101) {
			pr_info("[EPRJ-MUIC] Ret 101! --- USB Host Mode cable detected --- \n");
			i2c_smbus_write_byte_data(muic_client, INT_STAT, 0x1B);
			muic_proc_set_ap_usb();
			check_charging_mode();
		}
