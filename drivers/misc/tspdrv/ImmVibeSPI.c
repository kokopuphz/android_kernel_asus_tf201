...
#ifdef EPRJ_CHATTY
#define pr_eprj(x, ...)		pr_info(x, ##__VA_ARGS__)
#else
#define pr_eprj(x, ...)
#endif
...
#define DEBUG_MSG pr_eprj	// todo - define to something

#define PWM_PERIOD_DEFAULT              44000 //20.3KHz
#define PWM_DUTY_DEFAULT              (PWM_PERIOD_DEFAULT >> 1) //50%

unsigned int eprj_vibrator_duty = (PWM_PERIOD_DEFAULT >> 1);
VibeUInt32 g_nPWM_Freq = PWM_PERIOD_DEFAULT;

#define PWM_CLK_ENABLE tspdrv_control_pwm(1, eprj_vibrator_duty, PWM_PERIOD_DEFAULT);	// todo - define to something
...
VibeStatus ImmVibeSPI_ForceOut_AmpDisable(VibeUInt8 nActuatorIndex)
{
    ...
    if (g_bAmpEnabled)
    {
        pr_eprj("ImmVibeSPI_ForceOut_AmpDisable.\n");
...
VibeStatus ImmVibeSPI_ForceOut_AmpEnable(VibeUInt8 nActuatorIndex)
{
    ...
    if (!g_bAmpEnabled)
    {
        pr_eprj("ImmVibeSPI_ForceOut_AmpEnable.\n");
        g_bAmpEnabled = true;
		...
		{
			pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] Case : ELT Test Vibration\n");
			if(status == 0){
				pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] ELT : Register Setting \n");
				...
				pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] ELT HCTRL0 written data : 0x%x\n", I2C_data[0]);

				I2C_data[0] = tspdrv_i2c_read_byte_data(HCTRL4);
				pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] ELT HCTRL1 written data : 0x%x\n", I2C_data[0]);
				
				status = 1;
			}
			else{ //Normal Mode
				pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] ELT : just vibrate \n");

			}
		}
		else{
			pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] Case : Normal Vibration\n");
            if(status == 1){
                pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] Normal : Register Setting \n");
                ...
                I2C_data[0] = tspdrv_i2c_read_byte_data(SCTRL);
                pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] Normal HCTRL0 written data : 0x%x\n", I2C_data[0]);

                I2C_data[0] = tspdrv_i2c_read_byte_data(HCTRL4);
                pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] Normal HCTRL1 written data : 0x%x\n", I2C_data[0]);

				status =0;
            }
            else{
                pr_eprj("[ImmVibeSPI_ForceOut_AmpEnable] Normal : just vibrate \n");
		...
		pr_eprj("HCTRL0 written data : 0x%x\n", I2C_data[0]);

		I2C_data[0] = tspdrv_i2c_read_byte_data(HCTRL1);
		pr_eprj("HCTRL1 written data : 0x%x\n", I2C_data[0]);

		I2C_data[0] = tspdrv_i2c_read_byte_data(HCTRL2);
		pr_eprj("HCTRL2 written data : 0x%x\n", I2C_data[0]);

		I2C_data[0] = tspdrv_i2c_read_byte_data(HCTRL3);
		pr_eprj("HCTRL3 written data : 0x%x\n", I2C_data[0]);

		I2C_data[0] = tspdrv_i2c_read_byte_data(HCTRL4);
		pr_eprj("HCTRL4 written data : 0x%x\n", I2C_data[0]);
#endif
    }
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_Initialize(void)
{
    ...
    pr_eprj("ImmVibeSPI_ForceOut_Initialize.\n");
...
{
    pr_eprj("ImmVibeSPI_ForceOut_Terminate.\n");
    SYS_API_LEN_LOW;
#ifdef ISA1200_HEN_ENABLE        
    SYS_API_HEN_LOW;
#endif	
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_SetSamples(VibeUInt8 nActuatorIndex, VibeUInt16 nOutputSignalBitDepth, VibeUInt16 nBufferSizeInBytes, VibeInt8* pForceOutputBuffer)
{
    ...
    switch (nOutputSignalBitDepth)
    {
        case 8:
			...
			{
				pr_eprj("[ImmVibeSPI] ImmVibeSPI_ForceOut_SetSamples nBufferSizeInBytes =  %d \n", nBufferSizeInBytes );
				return VIBE_E_FAIL;
            }
    ...
    if(nForce == 0)
    {
        duty_ns = eprj_vibrator_duty;
    }
    else
    {
        duty_ns = ((nForce + 128) * g_nPWM_Freq) >> 8;
    }
//	pr_eprj("****** nForce : %d , duty_ns : %d ****\n", nForce, duty_ns);
