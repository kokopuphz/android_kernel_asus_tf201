...
#define pr_tps_warn_fmt(fmt) "[BATT][tps65200] warn:" fmt
...
#define pr_tps_warn(fmt, ...) \
	printk(KERN_WARNING pr_tps_warn_fmt(fmt), ##__VA_ARGS__)
...
#if FUNC_CALL_CHECK
#define CHECK_LOG() pr_tps_info("%s\n", __func__)
#else
...
extern bool is_alarm_initialized(void);
...
int tps_charger_dump_status(void)
{
	...
	if (chg_stat_enabled != 0) {
		...
		if (!(regh0 & 0x1C)) {
			...
			INFO("regh 0x00=%x, regh 0x06=%x, regh 0x07=%x,"
...
static int tps65200_dump_register(void)
{
	...
	INFO("regh 0x00=%x, regh 0x01=%x, regh 0x02=%x, regh 0x03=%x\n",
	...
	INFO("regh 0x04=%x, regh 0x05=%x, regh 0x06=%x\n",
			regh0, regh1, regh2);
	...
	INFO("regh 0x07=%x, regh 0x08=%x, regh 0x09=%x, regh 0x0A=%x\n",
			regh0, regh1, regh2, regh3);
	...
	INFO("regh 0x0B=%x, regh 0x0C=%x, regh 0x0D=%x, regh 0x0E=%x\n",
			regh0, regh1, regh2, regh3);
...
int tps_set_charger_ctrl(u32 ctl)
{
	...
	case POWER_SUPPLY_ENABLE_FAST_CHARGE:
		...
		INFO("regh 0x01=%x, regh 0x00=%x, regh 0x03=%x, regh 0x02=%x\n",
	...
	case POWER_SUPPLY_ENABLE_FAST_HV_CHARGE:
		...
		tps65200_i2c_read_byte(&regh2, 0x03);
		tps65200_i2c_read_byte(&regh3, 0x02);
		pr_tps_info("Switch charger ON (FAST_HV)\n");
		INFO("regh 0x01=%x, "
	...
	case CHECK_INT2:
		...
		break;
	case CHECK_INT3:
		tps65200_i2c_read_byte(&status, 0x0A);
		pr_tps_info("TPS65200 INT3 0x0Ah = %x\n", status);
		result = (int)status;
		break;
	case CHECK_CONTROL:
	...
	case POWER_SUPPLY_ENABLE_INTERNAL:
		...
		break;
	default:
		pr_tps_info("%s: Not supported battery ctr called.!\n", __func__);
		result = -EINVAL;
		break;
	}
	...
	default:
		...
		if (fault_bit & 0x80) {
			...
			if (fault_bit & 0x40) {
				...
				pr_tps_warn("CHG_INT is triggered "
					"by unhandled fault bit!\n");
				enable_irq(chg_int_data->gpio_chg_int);
			}
		}
	}
...
static ssize_t tps65200_status_show(struct device *dev
...
static DEVICE_ATTR(status, 0444, tps65200_status_show, NULL);
			...
			const struct i2c_device_id *id)
{
	...
	if (rc)
		pr_tps_err("device_create_file \"status\" failed\n");
