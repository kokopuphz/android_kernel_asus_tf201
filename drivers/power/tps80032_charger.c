...
int tps80032_charger_set_ctrl(u32 ctl)
{
	...
	if (tps80032_charger_initial < 0) {
		INFO("tps80032 charger not ready, state = %d!!\n", tps80032_charger_initial);
		return 0;
	}

	switch (ctl) {
	case POWER_SUPPLY_DISABLE_CHARGE:
		INFO("Switch charger OFF\n");
	...
	case OVERTEMP_VREG:
		...
		INFO("Switch charger OVERTEMP_VREG: regh 0x%X=%x\n", CHARGERUSB_VOREG, regh);
	...
	case UNDERTEMP_VREG:
		...
		tps80032_charger_regulation_voltage_set(regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VOREG, &regh);
		INFO("Switch charger UNDERTEMP_VREG: regh 0x%X=%x\n", CHARGERUSB_VOREG, regh);
	...
	case NORMALTEMP_VREG:
		...
		tps80032_charger_regulation_voltage_set(regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VOREG, &regh);
		INFO("Switch charger NORMALTEMP_VREG: regh 0x%X=%x\n", CHARGERUSB_VOREG, regh);
	...
	case NORMALTEMP_VREG_HV:
		...
		tps80032_charger_regulation_voltage_set(regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VOREG, &regh);
		INFO("Switch charger NORMALTEMP_VREG_HV: regh 0x%X=%x\n", CHARGERUSB_VOREG, regh);
	...
	case NORMALTEMP_VREG_HV4340:
		...
		tps80032_charger_regulation_voltage_set(regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VOREG, &regh);
		INFO("Switch charger NORMALTEMP_VREG_HV4340: regh 0x%X=%x\n", CHARGERUSB_VOREG, regh);

		break;
	case ENABLE_HIZ_CHG:
		INFO("Switch charger HIZ\n");
	...
	case NORMALTEMP_VREG_HV4320:
		...
		tps80032_charger_regulation_voltage_set(regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VOREG, &regh);
		INFO("Switch charger NORMALTEMP_VREG_HV4320: regh 0x%X=%x\n", CHARGERUSB_VOREG, regh);
	...
	case ALLTEMP_VSYS_DISABLE:
		...
		INFO("Switch charger ALLTEMP_VSYS_DISABLE: regh 0x%X=%x\n", CHARGERUSB_VSYSREG, regh);
	...
	case NORMALTEMP_VSYS_4400:
		...
		TPS80032_ID2_I2C_WRITE(CHARGERUSB_VSYSREG, regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VSYSREG, &regh);
		INFO("Switch charger NORMALTEMP_VSYS_4400: regh 0x%X=%x\n", CHARGERUSB_VSYSREG, regh);
	...
	case NORMALTEMP_VSYS_4440:
		...
		TPS80032_ID2_I2C_WRITE(CHARGERUSB_VSYSREG, regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VSYSREG, &regh);
		INFO("Switch charger NORMALTEMP_VSYS_4440: regh 0x%X=%x\n", CHARGERUSB_VSYSREG, regh);
	...
	case OVERTEMP_VSYS_4300:
		...
		TPS80032_ID2_I2C_WRITE(CHARGERUSB_VSYSREG, regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VSYSREG, &regh);
		INFO("Switch charger OVERTEMP_VSYS_4300: regh 0x%X=%x\n", CHARGERUSB_VSYSREG, regh);
	...
	case OVERTEMP_VSYS_4340:
		...
		TPS80032_ID2_I2C_WRITE(CHARGERUSB_VSYSREG, regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VSYSREG, &regh);
		INFO("Switch charger OVERTEMP_VSYS_4340: regh 0x%X=%x\n", CHARGERUSB_VSYSREG, regh);
	...
	case OVERTEMP_VSYS_4140:
		...
		TPS80032_ID2_I2C_WRITE(CHARGERUSB_VSYSREG, regh);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VSYSREG, &regh);
		INFO("Switch charger OVERTEMP_VSYS_4140: regh 0x%X=%x\n", CHARGERUSB_VSYSREG, regh);
	...
	case ENABLE_LIMITED_CHG:
		...
		INFO("Switch charger ON (LIMITED): regh 0x%x=%x low_chg=%u\n", CHARGERUSB_VICHRG, regh, tps80032_low_chg);
		break;
	case CLEAR_LIMITED_CHG:
		tps80032_low_chg = 0;
		TPS80032_ID2_I2C_WRITE(CHARGERUSB_VICHRG, 0x0A);
		TPS80032_ID2_I2C_READ(CHARGERUSB_VICHRG, &regh);
		INFO("Switch charger OFF (LIMITED): regh 0x%x=%x low_chg=%u\n", CHARGERUSB_VICHRG, regh, tps80032_low_chg);
		break;
	case SET_ICL_NORMAL:
		tps80032_icl_750 = 0;
		if (tps80032_charger_state == 2) {
			...
			INFO("Switch charger SET_ICL_NORMAL: regh 0x%x=0x%x\n", CHARGERUSB_CINLIMIT, regh);
		} else
			INFO("Switch charger SET_ICL_NORMAL \n");
		break;
	case SET_ICL750:
		tps80032_icl_750 = 1;
		if (tps80032_charger_state == 2) {
			TPS80032_ID2_I2C_UPDATE(CHARGERUSB_CINLIMIT, 0x0E, 0x3F);
			TPS80032_ID2_I2C_READ(CHARGERUSB_CINLIMIT, &regh);
			INFO("Switch charger SET_ICL750: regh 0x%x=0x%x\n", CHARGERUSB_CINLIMIT, regh);
		} else
			INFO("Switch charger SET_ICL750 \n");
		break;
	case CHECK_CHG:
	case CHECK_CONTROL:
	default:
...
static irqreturn_t int_chg_isr(int irq, void *dev_id)
{
	...
	if (ret < 0) {
		...
		INFO("%s():The status of CHARGERUSB_INT_STATUS is 0x%02x\n",
				__func__, chg_status);
...
static void chg_linch_work_func(struct work_struct *work)
{
	...
	INFO("%s():The status of LINEAR_CHRG_STS is 0x%02x\n",
