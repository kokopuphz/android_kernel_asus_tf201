			...
			case POWER_SUPPLY_TYPE_USB:
				switch (charging_mode) {
					...
					case CHARGING_EPRJUSBH:
						/* Limit USB Host power to 700mA */
					//	reg_val = ((chip->pdata->chgcc_soc700 << MAX8971_CHGCC_SHIFT) |
...
//		dev_dbg(&chip->client->dev, "MAX8971_REG_TEMPREG(0x%x) = 0x%02x\n", MAX8971_REG_TEMPREG, reg_val);
		...
		/* disable charge */
		pr_info("[EPRJ-MAX8971] Disable charge!!!\n");
...
//#define MAX77663_USB_VBUS_GPIO	257
//	gpio_set_value_cansleep(MAX77663_USB_VBUS_GPIO, 0);
...
static int max8971_is_charging(struct max8971_chip *info)
{
	...
	dev_info(&info->client->dev, "PRE - charging online = %d\n", info->chg_online);

	if (charging_mode != CHARGING_EPRJUSBH)
...
int max8971_is_charging_enable(void)
{
	...
	if (unlikely(charging_mode == CHARGING_EPRJUSBH)) {
		pr_info("[MAX8971CHG-EPRJ] %s = 1 (chg_dtls_val = 0x%02X)\n",
			...
			case POWER_SUPPLY_TYPE_USB:
				switch (charging_mode) {
					case CHARGING_MHL:
						...
						break;
					case CHARGING_EPRJUSBH:
						pr_info("Cable type is POWER_SUPPLY_TYPE_USB(EPRJUSBH)\n");
						//max8971_charger_wq(MAX8971_IRQ_PWRUP_OK, info);
						{
							chg_flag_muic = 1;
							...
							//max8971_charger_detail_irq(MAX8971_IRQ_PWRUP_OK, info, val);
...
//EPRJ: DETAILS2 IS READ-ONLY!!!			max8971_set_bits(info->client, MAX8971_REG_DETAILS2,
//									 MAX8971_CHG_DTLS_MASK, MAX8971_CHG_DTLS_FAST_CHARGE_CC);
							cancel_delayed_work(&info->monitor_work);
							schedule_delayed_work(&info->monitor_work, 2*HZ);

//							max8971_write_reg(info->client, MAX8971_REG_FCHGCRNT, 0x1f);
//							max8971_write_reg(info->client, MAX8971_REG_DCCRNT, 0x3f);
//							max8971_write_reg(info->client, MAX8971_REG_TOPOFF, 0x1e);
//							max8971_write_reg(info->client, MAX8971_REG_CHGCNTL1, 0x02);
						}
						break;
					default:
