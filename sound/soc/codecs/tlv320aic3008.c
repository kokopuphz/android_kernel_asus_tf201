...
#include <../arch/arm/mach-tegra/eprj_enru/htc_audio_power.h>
...
static struct pm_qos_request aud_cpu_minfreq_req;
...
static void eprj_soundboost(bool is_activated)
{
	if (is_activated)
		aic3008_config(ETERNITYPRJ_SOUNDBOOST_ON,
			ARRAY_SIZE(ETERNITYPRJ_SOUNDBOOST_ON));
	else
		aic3008_config(ETERNITYPRJ_SOUNDBOOST_OFF,
			ARRAY_SIZE(ETERNITYPRJ_SOUNDBOOST_OFF));
}
#endif
...
static int aic3008_suspend(struct snd_soc_codec *codec)
{
...
static struct snd_soc_codec_driver soc_codec_dev_aic3008_driver __refdata = {
	.probe =	aic3008_probe,
