...
#define AUD_ERR(fmt, ...) pr_err(fmt, ##__VA_ARGS__);
#define AUD_INFO(fmt, ...) pr_info(fmt, ##__VA_ARGS__);

#if AUDIO_DEBUG
#define AUD_DBG(fmt, ...) pr_info(LOG_TAG, fmt, ##__VA_ARGS__);
#else
		...
		struct snd_pcm_hw_params *params)
{
	...
	dai_flag |= SND_SOC_DAIFMT_NB_NF; // Normal bit clock + frame
...
static int tegra_aic3008_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	*/
	// Add controls
	err = snd_soc_add_codec_controls(codec, tegra_controls,
