...
#include "drivers/include/synaptics_i2c_rmi.h"
...
#if 0
static struct rfkill_gpio_platform_data eprj_bt_rfkill_pdata[] = {
	{
		.name		= "bt_rfkill",
		.shutdown_gpio	= 160,
...
static struct platform_device eprj_bt_rfkill_dev = {
	.name	= "rfkill_gpio",
	.id	= -1,
	.dev	= {
		.platform_data = &eprj_bt_rfkill_pdata,
	},
};
#endif

static struct resource eprj_bluesleep_resources[] = {
	[0] = {
		.name	= "gpio_host_wake",
		.start	= TEGRA_GPIO_PO5,
		.end	= TEGRA_GPIO_PO5,
		.flags	= IORESOURCE_IO,
	},
	[1] = {
...
static struct platform_device eprj_bluesleep_dev = {
	.name		= "bluesleep",
	.id		= -1,
	.num_resources	= ARRAY_SIZE(eprj_bluesleep_resources),
	.resource	= eprj_bluesleep_resources,
};
...
#if 0
static void __init eprj_bt_rfkill(void)
{
	platform_device_register(&eprj_bt_rfkill_dev);
	return;
}
#endif
static void __init eprj_bluesleep_init(void)
{
	eprj_bluesleep_resources[1].start = gpio_to_irq(TEGRA_GPIO_PO5);
	eprj_bluesleep_resources[1].end   = gpio_to_irq(TEGRA_GPIO_PO5);

	platform_device_register(&eprj_bluesleep_dev);
	return;
}
...
static struct tegra_i2c_platform_data enterprise_i2c5_platform_data = {
	.adapter_nr	= 4,
	.bus_count	= 1,
//	.bus_clk_rate	= { 400000 }, // EternityProject TEST: Why should it be at 100000 instead of 384000?
...
static struct htc_asoc_platform_data enru_eprjaudio_pdata;

static struct platform_device enterprise_audio_device = {
	.name	= "tegra-snd-aic3008",
	.id	= 0,
	.dev	= {
		.platform_data  = &enru_eprjaudio_pdata,
	},
};
// --- Audio tlv320aic3008 ---
...
static int eprjman_htcrc_1v2(void) { return 1; }

static int eprjman_htcrc_voltage_on(void)
{
	struct clk *csus_clk = NULL;
	struct clk *sensor_clk = NULL;
	int ret = -1;

	pr_info("[EPRJCAMERA] HTCRawchip: Powering on...\n");

	/* Get CSUS and VI_SENSOR clocks */
	csus_clk = clk_get(NULL, "csus");
	if (IS_ERR_OR_NULL(csus_clk))
	{
		pr_err("ERROR: Cannot get CSUS clock\n");
		csus_clk = NULL;
		return -1;
	}
	...
	{
		pr_err("ERROR: Cannot get VI_SENSOR clock\n");
		sensor_clk = NULL;
		return -1;
	}

	/* Clocks retrieved. Enable them! */
	clk_enable(csus_clk);
	clk_enable(sensor_clk);
	clk_set_rate(sensor_clk, 24000000);	/* 24MHz */

	/* Power ON sequence for HTC's rawchip */
	...
	/* PowerON sequence: Enable voltages */
...
static int eprjman_htcrc_voltage_off(void)
{
	struct clk *csus_clk = NULL;
	struct clk *sensor_clk = NULL;
...
static struct tegra_camera_rawchip_info tegra_rawchip_board_info = {
	...
	.camera_rawchip_power_on = eprjman_htcrc_voltage_on,
	.camera_rawchip_power_off = eprjman_htcrc_voltage_off,
	.rawchip_use_ext_1v2 = eprjman_htcrc_1v2,
};
...
static void __init enterprise_uart_init(void)
{
	...
	enterprise_bt_uart_pdata.bt_en = TEGRA_GPIO_PU0;
	enterprise_bt_uart_pdata.bt_cts_irq = TEGRA_GPIO_PO5;
...
static struct platform_device *eternityaudio_enru_devices[] __initdata = {
	&tegra_ahub_device,
	&tegra_dam_device0,
	&tegra_dam_device1,
	&tegra_dam_device2,
...
static void eternityaudio_board_init(void)
{
//	eternityaudio_enru_aicinit(&enru_eprjaudio_pdata);

printk("platform_add_devices eprjaudio\n");
	platform_add_devices(eternityaudio_enru_devices,
			ARRAY_SIZE(eternityaudio_enru_devices));
}
...
static struct synaptics_i2c_rmi_platform_data edge_ts_3k_data_XB[] = {
	{
		...
		.display_width = 720,
		.display_height = 1280,
		.notifyFinger = NULL, /* EternityProject: Do nothing. */
...
static int __init enterprise_touch_init(void)
{
	...
	synaptics_i2c_info_XB[0].irq = gpio_to_irq(TOUCH_GPIO_IRQ);
...
#define USE_EPRJBASEBAND_POSTRESUME
#ifdef USE_EPRJBASEBAND_POSTRESUME
/*
 * EPRJ Note: Okay, this is a duplicate and code duplication
 * is bad... but we're about to add some other code in there.
 */
static void eprjbaseband_usb_hsic_post_resume(void)
{
	pr_debug("%s has been called\n", __func__);
	baseband_xmm_set_power_status(BBXMM_PS_L0);
}
#endif
...
#ifdef USE_EPRJBASEBAND_POSTRESUME
	.post_resume = eprjbaseband_usb_hsic_post_resume,
#endif
...
static void cable_tegra_gpio_init(void)
{
	...
	if (cable_detect_pdata.usb_id_pin_gpio >= 0) {
		...
		if (gpio_request(
		    cable_detect_pdata.usb_id_pin_gpio,
		    "USB_ID_WAKE") < 0) {
...
#ifdef CONFIG_TEGRA_HDMI_MHL
static void eprj_register_mhl(void)
{
	int err;

	i2c_mhl_sii_info[0].irq =
		gpio_to_irq(EDGE_GPIO_MHL_INT);

	err = i2c_register_board_info(4,
			i2c_mhl_sii_info,
			ARRAY_SIZE(i2c_mhl_sii_info));
	if (err < 0)
		pr_warn("EPRJ: MHL registration failed!\n");
}
#endif

#ifdef CONFIG_EPRJ_CPUMANAGER
struct platform_device eprj_cpuman_device = {
	.name		= "eprj-cpuman",
	.id		= 0,
};
static int eprj_cpuman_register(void)
{
	int ret = 0;

	pr_info("[EPRJ-ENRU] Registering EPRJ T3 CPUManager\n");

	ret = platform_device_register(&eprj_cpuman_device);
	return ret;
};
#endif

static struct of_device_id dt_match_table[] __initdata = {
	{ .compatible = "simple-bus", },
	{}
};

struct of_dev_auxdata eprj_auxdata_lookup[] __initdata = {
//	OF_DEV_AUXDATA("nvidia,tegra20-gpio", TEGRA_GPIO_BASE, "tegra-gpio", NULL),
	OF_DEV_AUXDATA("nvidia,tegra30-pinmux", TEGRA_APB_MISC_BASE, "tegra30-pinctrl", NULL),
//	OF_DEV_AUXDATA("nvidia,tegra30-sdhci", 0x78000000, "sdhci-tegra.0", NULL),
//	OF_DEV_AUXDATA("nvidia,tegra30-sdhci", 0x78000200, "sdhci-tegra.1", NULL),
//	OF_DEV_AUXDATA("nvidia,tegra30-sdhci", 0x78000400, "sdhci-tegra.2", NULL),
//	OF_DEV_AUXDATA("nvidia,tegra30-sdhci", 0x78000600, "sdhci-tegra.3", NULL),
...
static void __init tegra_enterprise_init(void)
{
	...
	of_platform_populate(NULL, dt_match_table, eprj_auxdata_lookup, NULL);
...
//	tegra_enable_pinmux();
	tegra_smmu_init();
//	endeavoru_pinmux_init(); /* EternityProject: This is now handled by DT */
	enterprise_i2c_init();
	...
	if (properties_kobj) {
		if (htc_get_pcbid_info() >= PROJECT_PHASE_XC)
			err = sysfs_create_group(properties_kobj,
					&Aproj_properties_attr_group_XC);
		else
			err = sysfs_create_group(properties_kobj,
					&Aproj_properties_attr_group);
	}
	if (err < 0)
		pr_warn("Board properties sysfs registration failed!\n");

	endeavoru_suspend_init();
	eprj_enru_bpc_mgmt_init();
...
static const char *eternityprj_dt_board_compat[] = {
	"nvidia,endeavoru",
	NULL
};

void __init enru_init_external(void)
{
	tegra_enterprise_init();
}
EXPORT_SYMBOL_GPL(enru_init_external);
...
MACHINE_START(ENDEAVORU, "endeavoru")
	...
	.dt_compat	= eternityprj_dt_board_compat,
MACHINE_END
