...
#define ENABLE_NASTY_AND_SAD_HTC_THINGS
#ifdef ENABLE_NASTY_AND_SAD_HTC_THINGS
...
static int baseband_init(void)
{
	...
	/* EternityProject 13/04/2013 [START] */
#ifdef ENABLE_NASTY_AND_SAD_HTC_THINGS
	tegra_null_ulpi_init();
#endif

	wake_lock_init(&mdm_wake_lock, WAKE_LOCK_SUSPEND, "eprj_enrubaseband_lock");
	...
	ret = request_threaded_irq(irq, NULL, mdm_start_thread,
				   ...
				   "mdm_start", NULL);
	if (ret < 0) {
		pr_err("!!!EPRJBASEBAND_PLATFORM!!! request_threaded_irq ERROR!\n");
		return ret;
	}

	ret = enable_irq_wake(irq);
	if (ret) {
		pr_err("!!!EPRJBASEBAND_PLATFORM!!! enable_irq_wake ERROR!\n");
		free_irq(irq, NULL);
		return ret;
	}
...
int __init endeavoru_modem_init(void)
{
