...
static void radio_detect_work_handler(struct work_struct *work)
{
	...
	/* Sleep 30ms and then check if radio is OFF */
	msleep(30);

	radiopower = gpio_get_value(TEGRA_GPIO_PM4);
	if (!radiopower) {
		pr_debug("EPRJBASEBAND/PW2: Radio is off. "
			 "That means we aren't getting a "
			 "coredump interrupt.\n");
		return;
	}

	if (radio_detect_status != RADIO_STATUS_READY) {
		pr_debug("EPRJBASEBAND/PW2: Radio status: "
			 "%d. Returning.\n", radio_detect_status);
		return;
	}

	status = gpio_get_value(TEGRA_GPIO_PN2);
	pr_debug("EPRJBASEBAND/PW2: TEGRA_GPIO_PN2: %d\n", status);

	/* For XMM6260, FATAL happens when PN2 (coredump detect) is falling */
	if (status == 0)
		radio_detect_status = RADIO_STATUS_FATAL;

	if (radio_detect_status == RADIO_STATUS_FATAL)
	{
		char message[20] = "EPRJBASEBANDPW2=";
		char *envp[] 	 = { message, NULL };

		strncat(message, "FATAL", 5);
		pr_err("EPRJBASEBAND/PW2: Radio fatal! "
			"core dump uevent triggered!\n");
		kobject_uevent_env(&modem_info.modem_core_dump_kobj,
							KOBJ_ADD, envp);
	}
...
static irqreturn_t radio_det_irq(int irq, void *dev_id)
{
	...
	if (radio_detect_status < RADIO_STATUS_READY) {
		pr_err("EPRJBASEBAND/PW2: !!!Critical!!! "
			"Spurious IRQ detected!!!\n");
	} else {
		ret = gpio_get_value(TEGRA_GPIO_PN2);

		pr_debug("EPRJBASEBAND/PW2: CORE_DUMP_DETECT ");
		if (!ret)
			pr_debug("is falling\n");
		else
			pr_debug("is rising\n");
...
static void xmm_power2_step1(struct work_struct *work)
{
	...
	/* check for platform data */
	if (!pdata) {
		pr_debug("EPRJBASEBAND/PW2: pdata is NULL in step1!!!\n");
		return;
	}

	pr_debug("EPRJBASEBAND/PW2: Going to unregister HSIC\n");
	...
	pr_debug("EPRJBASEBAND/PW2: waiting X ms...\n");

	/* wait X ms */
	msleep(X);

	pr_debug("EPRJBASEBAND/PW2: Done. Setting IPC_HSIC_ACTIVE low.\n");
...
static void xmm_power2_work_func(struct work_struct *work)
{
	...
	if (!data || !data->pdata) {
		pr_debug("EPRJBASEBAND/PW2: work_func data is NULL!!\n");
		return;
	}
	pdata = data->pdata;
...
#ifdef EPRJ_FOR_ENRU
	if (xmm_power_drv_data.pdata == NULL) {
		pr_err("EPRJBASEBAND/PW2: No platform data in probe func\n");
		return -EINVAL;
	}

	kobj_hsic_device = kobject_create_and_add(
				"modem-dump-kobj", NULL);
	if (!kobj_hsic_device) {
		pr_err("EPRJBASEBAND/PW2: Can't create kobj_hsic_device\n");
		return -ENOMEM;
	}

	/* Radio Detection (XMM6260) */
	radio_detect_status = RADIO_STATUS_UNKNOWN;
	err_radio = request_irq(gpio_to_irq(TEGRA_GPIO_PN2),
				radio_det_irq, IRQF_TRIGGER_FALLING,
				"RADIO_DETECT", &modem_info);
	if (err_radio < 0)
		pr_err("EPRJBASEBAND/PW2: Failed to request RADIO_DETECT IRQ\n");

	radio_detect_status = RADIO_STATUS_READY;

	modem_kset_radio = kset_create_and_add(
				"modem_coreDump", NULL, kobj_hsic_device);
	if (!modem_kset_radio) {
		kobject_put(kobj_hsic_device);
		pr_err("EPRJBASEBAND/PW2: Error while allocating "
			"modem_kset_radio. Init failed.\n");
		return -ENOMEM;
	}

	pr_debug("EPRJBASEBAND/PW2: kobj_hsic_device and modem_kset_radio "
		 "have been created successfully.\n"
		 "BBPW2: Initializing the core dump and adding to kobject.\n");
	modem_info.modem_core_dump_kobj.kset = modem_kset_radio;

	err_radio = kobject_init_and_add(&modem_info.modem_core_dump_kobj,
			&htc_modem_ktype, NULL, "htc_modem_radioio_det");
	if (err_radio) {
		pr_err("EPRJBASEBAND/PW2: Error while allocating "
			"modem_core_dump_kobj on htc_modem_ktype.\n");
...
static int xmm_power2_remove(struct platform_device *device)
{
...
#ifdef EPRJ_FOR_ENRU
	radio_detect_status = RADIO_STATUS_UNKNOWN;
	free_irq(gpio_to_irq(TEGRA_GPIO_PN2), &modem_info);

	if (modem_kset_radio) {
		pr_debug("EPRJBASEBAND/PW2: Freeing modem_kset_radio\n");
		kobject_put(&modem_info.modem_core_dump_kobj);
		kset_unregister(modem_kset_radio);
		modem_kset_radio = NULL;
	}
	...
	if (kobj_hsic_device) {
		pr_debug("EPRJBASEBAND/PW2: Freeing kobj_hsic_device\n");
		kobject_put(kobj_hsic_device);
	}
...
static int __init xmm_power2_init(void)
{
	...
	wake_lock_init(&modem_info.modem_wake_lock,
			WAKE_LOCK_SUSPEND, "htc_modem_6260");
#endif
