...
static int emmc_suspend_gpiocfg(void)
{
	...
	ret = gpio_request(TEGRA_GPIO_PCC4, "SDMMC4_CLK");
	if (ret < 0) {
		pr_err("%s: Failed to request GPIO SDMMC4_CLK (return:%d)\n",
			__func__, ret);
		return -1;
	}

	ret = gpio_direction_output(TEGRA_GPIO_PCC4, 0);
	if (ret < 0) {
		pr_err("%s: Failed setting GPIO SDMMC4_CLK direction (return:%d)\n",
			__func__, ret);
		gpio_free(TEGRA_GPIO_PCC4);
		return -1;
	}
	tegra_pinmux_set_pullupdown(
		TEGRA_PINGROUP_SDMMC4_CLK, TEGRA_PUPD_NORMAL);
	tegra_gpio_enable(TEGRA_GPIO_PCC4);
...
static void emmc_resume_gpiocfg(void)
{
	tegra_pinmux_set_pullupdown(
		TEGRA_PINGROUP_SDMMC4_CLK, TEGRA_PUPD_NORMAL);
	tegra_gpio_disable(TEGRA_GPIO_PCC4);
	gpio_free(TEGRA_GPIO_PCC4);
}
...
static int __init enterprise_wifi_init(void)
{
	...
	enterprise_wlan_data.irq = gpio_to_irq(ENTERPRISE_WLAN_WOW);
