...
#ifdef HS_LOG
#undef HS_LOG
#define HS_LOG(x...) pr_info("[EPRJ_HSUART]" x)
#endif
...
/* BAD HACK -- HACK -- HACK -- ETERNITYPROJECT -- HACK -- HACK -- HACK */
...
//	}
	...
	hr_msleep(22);
	if (hs_read_aid() == 0) {
		/* That means we've detected a PL2303 chip */
...
static int htc_headset_1wire_probe(struct platform_device *pdev)
{
	...
	HS_LOG("Entering 1-wire probing function");
	...
	HS_LOG("Your 1-wire TTY device is %s", hi->pdata.onewire_tty_dev);
	hs_1wire_register();
