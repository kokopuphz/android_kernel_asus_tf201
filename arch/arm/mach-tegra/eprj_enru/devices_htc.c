...
int __init parse_tag_IMEI(const struct tag *tags)
{
	...
	if (!find) {
		pr_err("[IMEI] error: IMEI ATAG not found\n");
		return -1;
	} else {
		unsigned char *dptr = (unsigned char *)(&t->u);
		memcpy(IMEI, dptr, 16);
	}
/* EternityProject, 22/03/2013: Don't write plain IMEI on kmsg!!!
...
static int __init parse_tag_hsnr_calibration(const struct tag *tag)
{
	pr_info("[EPRJ] HSNR Calibration ATAG found. Ignoring.\n");
	return 0;
}
__tagtable(ATAG_HSNR, parse_tag_hsnr_calibration);
#endif
...
{
	if (strict_strtoul(s, 16, &radio_flag) < 0)
		return -EINVAL;
	return 1;
}
...
{
	if (strict_strtoul(s, 16, &kernel_flag) < 0)
		return -EINVAL;
	return 1;
}
...
{
	if (strict_strtoul(s, 16, &extra_kernel_flag) < 0)
		return -EINVAL;
	return 1;
}
...
static long unsigned int usb_ats = 0;
int __init board_ats_init(char *s)
{
	if (strict_strtoul(s, 16, &usb_ats) < 0)
		return -EINVAL;
	return 1;
}
		...
		unsigned long event, void *data)
{
	...
	pr_info("kernel_restart(cmd=%s) - triggered with task: %s (%d:%d)\n",
			(char*)(data ? data : "<null>"),
			(char*)current->comm, current->tgid, current->pid);
...
static int __init eprj_dt_probe(void)
{
	unsigned long dt_root;
	__be32 *prop;
	unsigned long len;

	pr_info("EternityProject DeviceTree compatibility for HTC: START\n");

	dt_root = of_get_flat_dt_root();

	pr_info("Getting values...\n");
	/* This is the actual probing */
	prop = of_get_flat_dt_prop(dt_root, "als-calibration", &len);
	if (prop) {
		als_kadc = of_read_number(prop, len / 4);
		pr_info("ALS calibration found! 0x%x \n", als_kadc);
	}

	prop = NULL; len = 0;
	prop = of_get_flat_dt_prop(dt_root, "proximity-calib-low", &len);
	if (prop) {
		ps_kparam1 = of_read_number(prop, len / 4);
		pr_info("Proximity calibration (LOW) found! 0x%x \n", ps_kparam1);
	}

	prop = NULL; len = 0;
	prop = of_get_flat_dt_prop(dt_root, "proximity-calib-high", &len);
	if (prop) {
		ps_kparam2 = of_read_number(prop, len / 4);
		pr_info("Proximity calibration (HIGH) found! 0x%x \n", ps_kparam2);
	}

	prop = NULL; len = 0;
	prop = of_get_flat_dt_prop(dt_root, "psensor-type", &len);
	if (prop) {
		ps_type = of_read_number(prop, len / 4);
		pr_info("Proximity chip type found! Type: %d\n", ps_type);
	}

	pr_info("EternityProject DeviceTree compatibility: END\n");

	return 0;
}
early_initcall(eprj_dt_probe);

unsigned get_last_reboot_params_battery_level(void)
{
	unsigned ret = get_last_htc_reboot_params()->battery_level;
	if (!ret)
		return 0;
	return ret;
}
