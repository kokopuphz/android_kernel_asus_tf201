...
#include "../include/synaptics_i2c_rmi.h"
#include <linux/slab.h>
#include "../include/rmi.h"
#include <asm/gpio.h>
#include "../../include/mach/cable_detect.h"
#include <linux/input/mt.h>
#include "../include/pl_sensor.h"
#include "../include/cm3629.h"
...
#ifdef EPRJ_CHATTY
#define TP_TAG "[TP]"
#define TP_PRINT(c, ...) pr_info(TP_TAG c, ##__VA_ARGS__)
#define TP_PRINT2(c, ...) pr_info(c, ##__VA_ARGS__)
#else
#define TP_PRINT(c, ...)
#define TP_PRINT2(c, ...)
#endif
...
static void syn_page_select(struct i2c_client *client, uint8_t page)
{
	...
	if (page ^ ts->page_select) {
		...
		ts->page_select = page;
/*		TP_PRINT(KERN_INFO "TOUCH: Page Select: %s: %d\n", __func__, ts->page_select); */
	}
...
static int i2c_syn_read(struct i2c_client *client, uint16_t addr, uint8_t *data, uint16_t length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		TP_PRINT(KERN_INFO "[TP] i2c_read retry over %d\n",
			SYN_I2C_RETRY_TIMES);
		return -EIO;
	}
...
static int i2c_syn_write(struct i2c_client *client, uint16_t addr, uint8_t *data, uint16_t length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		TP_PRINT(KERN_INFO "[TP] i2c_write retry over %d\n",
			SYN_I2C_RETRY_TIMES);
		return -EIO;
	}
...
int i2c_rmi_write(uint16_t addr, uint8_t *data, uint16_t length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		pr_err("[TP] i2c_write retry over %d\n",
			SYN_I2C_RETRY_TIMES);
		return -EIO;
	}
...
static int i2c_syn_error_handler(struct synaptics_ts_data *ts, uint8_t reset, char *reason, const char *fun_name)
{
	...
	if (reason && fun_name)
		pr_err("[TP] TOUCH_ERR: I2C Error: %s:%s, reset = %d\n", fun_name, reason, reset);
	else
		pr_err("[TP] %s: rason and fun_name can't be null\n", __func__);

	if (reset) {
		if (ts->power) {
			ret = ts->power(0);
			if (ret < 0)
				pr_err("[TP] TOUCH_ERR: synaptics i2c error handler power off failed\n");
			msleep(10);
			ret = ts->power(1);
			if (ret < 0)
				pr_err("[TP] TOUCH_ERR: synaptics i2c error handler power on failed\n");
			ret = synaptics_init_panel(ts);
			if (ret < 0)
				pr_err("[TP] TOUCH_ERR: synaptics i2c error handler init panel failed\n");
		} else if (ts->gpio_reset) {
			gpio_direction_output(ts->gpio_reset, 0);
			msleep(1);
			gpio_direction_output(ts->gpio_reset, 1);
			TP_PRINT(KERN_INFO "[TP] %s: synaptics touch chip reseted.\n", __func__);
		}
...
static int wait_flash_interrupt(struct synaptics_ts_data *ts, int attr)
{
	...
	for (i = 0; i < 5; i++) {
#ifdef SYN_FLASH_PROGRAMMING_LOG /* remember to activate EPRJ_CHATTY */
		TP_PRINT("ATT: %d\n", gpio_get_value(attr));
#endif
	...
	if (i == 5 && syn_panel_version == 0) {
		...
		pr_err("wait_flash_interrupt: interrupt over time!\n");
	...
	/* check = 0x80 */
	if (data != 0x80) {
		pr_err("[TP] wait_flash_interrupt: block config fail!\n");
		return SYN_PROCESS_ERR;
	}
	return 0;
}
...
#ifdef SYN_FLASH_PROGRAMMING_LOG
		TP_PRINT("[TP] config_crc = %X, flash_crc = %X\n", config_crc, flash_crc);
		for (j = 0; j < 0x11; j++)
			TP_PRINT(KERN_INFO " %d:%X ", j, data[j]);
		TP_PRINT(KERN_INFO "\n");
	}
#endif
...
static int syn_config_update(struct synaptics_ts_data *ts, int attr)
{
	...
	TP_PRINT(KERN_INFO "[TP] CRC = %X\n" , syn_crc((uint16_t *)ts->config, SYN_CONFIG_SIZE / 2 - 2));

	if (ts->tw_pin_mask == 0) {
		ret = enable_flash_programming(ts, attr);
		if (ret < 0) {
			TP_PRINT(KERN_INFO "[TP] syn_config_update: Enable flash programming fail!\n");
			return disable_flash_programming(ts, ret);
		}

		ret = syn_pdt_scan(ts, SYN_BL_PAGE);
		if (ret < 0) {
			TP_PRINT(KERN_INFO "[TP] syn_config_update: pdt scan failed\n");
			return disable_flash_programming(ts, ret);
		}
	}
	...
	for (retry = 0; retry < 3; retry++) {
		...
		if (ret < 0) {
#ifdef SYN_FLASH_PROGRAMMING_LOG
			TP_PRINT("syn_config_update: Program config fail %d!\n", retry + 1);
#endif
			continue;
		}
...
static int syn_get_tw_vendor(struct synaptics_ts_data *ts, int attr)
{
	...
	ret = enable_flash_programming(ts, attr);
	if (ret < 0) {
		TP_PRINT(KERN_INFO "[TP] Enable flash programming fail!\n");
		return disable_flash_programming(ts, -1);
	}
	...
	TP_PRINT(KERN_INFO "[TP] tw mask = %X %X , %X\n", data[0], data[1], ts->tw_pin_mask);
	...
	TP_PRINT(KERN_INFO "[TP] tw vendor= %x %x\n", data[1], data[0]);
...
static int synaptics_input_register(struct synaptics_ts_data *ts)
{
	...
	if (ts->input_dev == NULL) {
		ret = -ENOMEM;
		pr_err("[TP] TOUCH_ERR: %s: Failed to allocate input device\n", __func__);
		return ret;
	}
	...
	TP_PRINT(KERN_INFO "[TP] input_set_abs_params: mix_x %d, max_x %d, min_y %d, max_y %d\n",
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' &&
		...
		syn_reg_addr = addr;
		TP_PRINT("%s: set syn_reg_addr is: 0x%X\n",
						__func__, syn_reg_addr);
		if (buf[0] == 'w' && buf[5] == ':' && buf[9] == '\n') {
			memcpy(buf_tmp, buf + 6, 3);
			ret = strict_strtoul(buf_tmp, 16, &addr);
			write_da = addr;
			TP_PRINT("[TP] write addr: 0x%X, data: 0x%X\n",
	...
	if (unlock == 2 && ts->first_pressed && ts->pre_finger_data[0][0] < 2) {
		...
		if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
			...
			if (ts->energy_ratio_relaxation) {
				if (!ts->finger_count) {
					...
					if (ret < 0)
						return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
					ts->enable_ERR = 0;
					TP_PRINT(KERN_INFO "[TP] Disable ERR on syn_unlock_store\n");
				}
			}
			...
			if (ts->saturation_bef_unlock) {
				...
				if (ret < 0)
					return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:4", __func__);
				TP_PRINT(KERN_INFO "[TP] %s: unlock confirmed. set saturation: %x\n"
			...
			if (ts->multitouch_calibration) {
				...
				TP_PRINT(KERN_INFO "[TP] %s: Touch Calibration Confirmed, rezero\n", __func__);
			}
#endif
			if (ts->large_obj_check) {
				...
				TP_PRINT(KERN_INFO "[TP] %s: unlock confirmed. set large obj suppression: %x\n"
			...
			if (ts->segmentation_bef_unlock) {
				...
				TP_PRINT(KERN_INFO "[TP] %s: unlock confirmed. set segmentation aggressiveness: %x\n"
			...
			if (ts->threshold_bef_unlock) {
				...
				TP_PRINT(KERN_INFO "[TP] %s: unlock confirmed. set Z Touch threshold: %x\n"
		...
		struct device_attribute *attr, char *buf)
{
		...
		for (j = 0; j < ts->num_function; j++) {
			if (((ts->address_table[j].control_base >> 8) & 0xFF) == i) {
				...
				if (temp_func_cmd != ts->num_function) {
					...
					TP_PRINT(KERN_INFO "[TP] page%d has command function, function: %X\n"
						, i, ts->address_table[temp_func_cmd].function_type);
				} else {
					size = ts->address_table[temp_func_query].query_base -
						ts->address_table[j].control_base;
					TP_PRINT(KERN_INFO "[TP] page%d has no command function, use query function, function: %X\n"
				...
				length += size;
				TP_PRINT(KERN_INFO "[TP] Size: %x, Length: %x\n", size, length);
				break;
			}
		}
	}
	...
	TP_PRINT(KERN_INFO "");
	for (i = 0; i < length; i++) {
		TP_PRINT(KERN_INFO "%2.2X ", ts->config_table[i]);
		if ((i % 16) == 15)
			TP_PRINT(KERN_INFO "\n");
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < 20; i++) {
		if (buf[i] == ',' || buf[i] == '\n') {
			...
			if (i - j <= 5)
				...
				TP_PRINT(KERN_INFO "[TP] buffer size is over 5 char\n");
				return count;
			}
			j = i + 1;
			if (k < 4) {
	...
	if (k == 4) {
		...
		TP_PRINT(KERN_INFO "[TP] %d, %d, %d, %d\n",
			ts->layout[0], ts->layout[1], ts->layout[2], ts->layout[3]);
		input_unregister_device(ts->input_dev);
		synaptics_input_register(ts);
	} else
		TP_PRINT(KERN_INFO "[TP] ERR@%d, %d, %d, %d\n",
			ts->layout[0], ts->layout[1], ts->layout[2], ts->layout[3]);
	return count;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (value) {
		...
		if (ret == 0) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: interrupt enable: %x\n", __func__, ts->intr_bit);
			if (ret)
...
static enum hrtimer_restart synaptics_ts_timer_fake_event_func(struct hrtimer *timer)
{
	...
	if ((dx_fake > 0 ? X_tmp <= X_fake_E : dx_fake ? X_tmp >= X_fake_E : 0) ||
		...
		input_sync(ts->input_dev);
		i = 0;
		TP_PRINT(KERN_INFO "[TP]End of fake event\n");
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!i) {
		...
		TP_PRINT(KERN_INFO "hrtimer_init\n");
		i++;
	}
	...
	while (1) {
		if (buf[i] == ',' || buf[i] == '\n') {
			...
			if (i - j <= 5)
				...
				TP_PRINT("buffer size is over 5 char\n");
				return count;
			}
			j = i + 1;
...
static int register_sr_touch_device(void)
{
	...
	if (ts->sr_input_dev == NULL) {
		pr_err("[TP][TOUCH_ERR]%s: Failed to allocate SR input device\n", __func__);
...
/*	set_bit(KEY_APP_SWITCH, ts->sr_input_dev->keybit); */
	...
	TP_PRINT(KERN_INFO "[TP][SR]input_set_abs_params: mix_x %d, max_x %d,"
		" min_y %d, max_y %d\n", ts->layout[0],
	...
	if (input_register_device(ts->sr_input_dev)) {
		...
		pr_err("[TP][SR][TOUCH_ERR]%s: Unable to register %s input device\n",
						...
						const char *buf, size_t count)
{
	...
	if (buf[0]) {
		if (ts->sr_input_dev)
			TP_PRINT(KERN_INFO "[TP]%s: SR device already exist!\n", __func__);
		else
			TP_PRINT(KERN_INFO "[TP]%s: SR touch device enable result:%X\n", __func__, register_sr_touch_device());
	}
	return count;
}
...
static int synaptics_touch_sysfs_init(void)
{
	...
	if (android_touch_kobj == NULL) {
		pr_err("[TP] TOUCH_ERR: %s: subsystem_register failed\n", __func__);
		ret = -ENOMEM;
		return ret;
	}
	syn_reg_addr = 0;
	...
	if (ret) {
		TP_PRINT(KERN_INFO "[TP]%s: Failed to obtain touchpad IRQ %d. Code: %d.", __func__, ts->gpio_irq, ret);
		return ret;
	}
	if (ts->gpio_reset) {
		ret = gpio_request(ts->gpio_reset, "synaptics_reset");
		if (ret)
			TP_PRINT(KERN_INFO "[TP]%s: Failed to obtain reset pin: %d. Code: %d.", __func__, ts->gpio_reset, ret);
	}
	ret = gpio_export(ts->gpio_irq, true);
	if (ret) {
		TP_PRINT(KERN_INFO "[TP]%s: Failed to "
			"export ATTN gpio!\n", __func__);
		ret = 0;
	} else {
		...
		if (ret) {
			...
			TP_PRINT(KERN_INFO "[TP]%s: Exported GPIO %d.", __func__, ts->gpio_irq);
		}
	}
#endif
	return 0;
...
static int synaptics_init_panel(struct synaptics_ts_data *ts)
{
	...
	if (ts->pre_finger_data[0][0] < 2) {
		if (ts->large_obj_check) {
			...
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
			TP_PRINT(KERN_INFO "[TP] %s: set large obj suppression register to: %x\n", __func__, ts->default_large_obj & 0x7F);
		}
		...
		if (ts->segmentation_bef_unlock) {
			...
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:3", __func__);
			TP_PRINT(KERN_INFO "[TP] %s: set segmentation aggressiveness to: %x\n", __func__, ts->segmentation_bef_unlock);
		}
		...
		if (ts->threshold_bef_unlock) {
			...
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:4", __func__);
			TP_PRINT(KERN_INFO "[TP] %s: set Z Touch threshold to: %x\n", __func__, ts->threshold_bef_unlock);
		}
		...
		if (ts->saturation_bef_unlock) {
			...
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "saturation capacitance", __func__);
			TP_PRINT(KERN_INFO "[TP] %s: set saturation to: %x\n", __func__, ts->saturation_bef_unlock);
			ret = i2c_syn_write_byte_data(ts->client,
				get_address_base(ts, 0x54, COMMAND_BASE), 0x04);
			if (ret < 0)
				return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:5", __func__);
	...
	if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
		if (ts->pre_finger_data[0][0] >= 2 || ts->mfg_flag == 1) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: Touch init: set fast relaxation to 0x0\n", __func__);
		}
	}
#endif
...
static void synaptics_ts_finger_func(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		...
		if (ts->debug_log_level & BIT(0)) {
			TP_PRINT(KERN_INFO "[TP] Touch:");
			for (i = 0; i < sizeof(buf); i++)
				TP_PRINT(KERN_INFO " %2x", buf[i]);
			TP_PRINT(KERN_INFO "\n");
		}
		...
		if (finger_pressed == 0 /*||
			...
			if (ts->energy_ratio_relaxation) {
				if ((ts->pre_finger_data[0][0] == 2) && (ts->enable_ERR)) {
					...
					if (ret < 0)
						i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "w:2", __func__);
					ts->enable_ERR = 0;
					TP_PRINT(KERN_INFO "[TP] Disable ERR on Finger leave\n");
				}
			}
			...
			if (ts->debug_log_level & BIT(1))
				TP_PRINT(KERN_INFO "[TP] Finger leave\n");
		...
		if (ts->pre_finger_data[0][0] < 2 || finger_pressed) {
			...
			if (ts->debug_log_level & BIT(16))
				TP_PRINT(KERN_INFO "[TP] ts->grip_suppression: %x, ts->ambiguous_state: %x\n",
...
static void synaptics_ts_status_func(struct synaptics_ts_data *ts)
{
	...
	if (ret < 0) {
		...
		data &= 0x0F;
		TP_PRINT("Device Status = %x\n", data);
		if (data == 1) {
			mutex_lock(&syn_mutex);
			ts->page_select = 0;
			mutex_unlock(&syn_mutex);
			TP_PRINT("TOUCH: Page Select: %s: %d\n", __func__, ts->page_select);
			ret = synaptics_init_panel(ts);
			if (ret < 0)
				TP_PRINT("%s: synaptics_init_panel fail\n", __func__);
		}
	}
...
/*              TP_PRINT(KERN_INFO "[TP] Irq start time = %ld.%06ld s\n",
...
/*                              TP_PRINT(KERN_INFO "[TP] Irq finish time = %ld.%06ld s\n",
					timeEnd.tv_sec, timeEnd.tv_nsec/1000);*/
                                TP_PRINT(KERN_INFO "[TP] Touch latency = %ld us\n", timeDelta.tv_nsec/1000);
                        }
		}
		if (buf & get_address_base(ts, 0x01, INTR_SOURCE))
			synaptics_ts_status_func(ts);
...
static void cable_tp_status_handler_func(int connect_status)
{
	...
	TP_PRINT(KERN_INFO "[TP] Touch: cable change to %d\n", connect_status);
	...
	unsigned long status, void *unused)
{
	...
	TP_PRINT(KERN_INFO "[TP] psensor status %d -> %lu\n",
...
static int syn_pdt_scan(struct synaptics_ts_data *ts, int num_page)
{
	...
	if (ts->address_table == NULL) {
		...
		if (ts->address_table == NULL) {
			pr_err("syn_pdt_scan: memory allocate fail\n");
			return -ENOMEM;
		}
		TP_PRINT("syn_pdt_scan: memory allocate success. ptr: %p\n", ts->address_table);
	}

	TP_PRINT("synaptics: %d function supported\n", ts->num_function);
	for (i = 0; i < num_page; i++) {
		for (j = 0; j < num_function[i]; j++) {
			...
			TP_PRINT("Query: %2.2X, Command: %4.4X, Control: %2X, Data: %2X, INTR: %2X, Funtion: %2X\n",
...
static int syn_get_version(struct synaptics_ts_data *ts)
{
	...
	TP_PRINT(KERN_INFO "[TP] %s: panel_version: %x\n", __func__, syn_panel_version);
	...
	TP_PRINT(KERN_INFO "[TP] %s: config version: %x\n", __func__, ts->config_version);
...
/*	ret = i2c_syn_read(ts->client, get_address_base(ts, 0x01, QUERY_BASE) + 2, data, 2);
	...
	TP_PRINT("%s: panel_version: %x\n", __func__, syn_panel_version);

	ret = i2c_syn_read(ts->client, get_address_base(ts, 0x34, CONTROL_BASE), data, 4);
	if (ret < 0)
		return ret;
	ts->config_version = data[0] << 24 | data[1] << 16 | data[2] << 8 | data[3];
	TP_PRINT("%s: config version: %x\n", __func__, ts->config_version);
*/
	...
	if ((data[0] & 0x07) == 5)
		...
		TP_PRINT(KERN_INFO "[TP] %s: number of fingers not define: %x\n",
			__func__, data[0] & 0x07);
		return SYN_PROCESS_ERR;
	}

	TP_PRINT(KERN_INFO "[TP] %s: finger_support: %d\n", __func__, ts->finger_support);
	...
	if (get_address_base(ts, 0x54, FUNCTION)) {
		...
		if (buf == NULL) {
			TP_PRINT(KERN_INFO "[TP] %s: memory allocate fail\n", __func__);
			return -ENOMEM;
		}
		if (ts->packrat_number < SYNAPTICS_FW_3_2_PACKRAT)
		...
		TP_PRINT(KERN_INFO "[TP] %s: ts->relaxation: %d\n", __func__, ts->relaxation);
	...
	if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
		if (ts->large_obj_check) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: ts->default_large_obj: %x\n", __func__, ts->default_large_obj);
		}
		...
		if (ts->segmentation_bef_unlock) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: ts->segmentation_aft_unlock: %x\n", __func__, ts->segmentation_aft_unlock);
		}
		...
		if (ts->threshold_bef_unlock) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: ts->z_threshold_aft_unlock: %x\n", __func__, ts->threshold_aft_unlock);
		}
		...
		if (ts->saturation_bef_unlock) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: ts->saturation_aft_unlock: %x\n", __func__, ts->saturation_aft_unlock);
		}
	}
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TP_PRINT("%s: enter", __func__);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		pr_err("[TP] TOUCH_ERR: synaptics_ts_probe: need I2C_FUNC_I2C\n");
		ret = -ENODEV;
	...
	if (pdata == NULL) {
		TP_PRINT(KERN_INFO "[TP] pdata is NULL\n");
	...
	for (i = 0; i < 10; i++) {
		...
		if (data & 0x44) {
			msleep(20);
#ifdef SYN_FLASH_PROGRAMMING_LOG
			TP_PRINT("synaptics probe: F01_data: %x touch controller stay in bootloader mode!\n", data);
#endif
		} else if (data & 0x40) {
			TP_PRINT("TOUCH_ERR: synaptics probe: F01_data: %x touch controller stay in bootloader mode!\n", data);
			goto err_detect_failed;
		} else
			break;
	}
	...
	if (i == 10) {
		uint8_t num = 0;
		TP_PRINT(KERN_INFO "[TP] synaptics probe: touch controller doesn't enter UI mode! F01_data: %x\n", data);

		if (syn_pdt_scan(ts, SYN_BL_PAGE) < 0) {
			pr_err("[TP] TOUCH_ERR: PDT scan fail\n");
		...
		if (pdata) {
			while (pdata->default_config != 1) {
				if (pdata->default_config == 0) {
					pr_err("[TP] TOUCH_ERR: touch controller stays in bootloader mode "
			...
			if (ret < 0) {
				...
				TP_PRINT("syn_config_update success\n");
			else
				TP_PRINT("Warning: syn_config_update: the same "
	...
	if (syn_get_version(ts) < 0) {
		pr_err("[TP] TOUCH_ERR: syn_get_version fail\n");
		goto err_init_failed;
	}

	if (pdata) {
		while (pdata->packrat_number && pdata->packrat_number > ts->packrat_number) {
			TP_PRINT(KERN_INFO "[TP] pdata++, synaptics_ts_probe: pdata->version = %x, pdata->packrat_number = %d,"
		...
		TP_PRINT(KERN_INFO "[TP] synaptics_ts_probe: pdata->version = %x, pdata->packrat_number = %d,"
				" pdata->sensor_id = %x\n", pdata->version, pdata->packrat_number, pdata->sensor_id);

		if (ts->packrat_number < SYNAPTICS_FW_3_2_PACKRAT) {
			if ( NULL == pdata ) {
				pr_err("[TP] TOUCH_ERR: get null platform data\n");
				goto err_init_failed;
			}
		} else {
...
/*		if (pdata->abs_x_max  == 0 && pdata->abs_y_max == 0) {
	...
	if (ret < 0) {
		...
		TP_PRINT(KERN_INFO "[TP] syn_config_update success\n");
	else
		TP_PRINT(KERN_INFO "[TP] syn_config_update: the same config version and CRC\n");
#else
	if (pdata->tw_pin_mask) {
		ret = disable_flash_programming(ts, 0);
		if (ret < 0) {
			pr_err("[TP] TOUCH_ERR: disable_flash_programming fail\n");
			goto err_init_failed;
		}
	}
#endif
	...
	if (pdata->customer_register[CUS_REG_BASE]) {
		...
		TP_PRINT("Loads customer register\n");
	}
#endif

	if (syn_get_information(ts) < 0) {
		pr_err("[TP] TOUCH_ERR: syn_get_information fail\n");
	...
	if(pdata->display_width && pdata->display_height){
		TP_PRINT(KERN_INFO "[TP] Load display resolution: %dx%d\n", pdata->display_width, pdata->display_height);
...
/*
 * EternityProject: Remove tampering detection
 *
 *	if(get_tamper_sf()==0) {
 *		ts->debug_log_level |= BIT(3);
 *		TP_PRINT(KERN_INFO "[TP] Debug log level=0x%02X\n", ts->debug_log_level);
 *	}
 */
	if (get_address_base(ts, 0x19, FUNCTION)) {
		...
		for (i = 0; i < ts->key_number; i++) {
			ts->key_postion_x[i] =
				(ts->layout[1] - ts->layout[0]) * (i * 2 + 1) / (ts->key_number * 2)
				+ ts->layout[0];
			TP_PRINT("ts->key_postion_x[%d]: %d\n",
				i, ts->key_postion_x[i]);
		}
		ts->key_postion_y = ts->layout[2] +
			(21 * (ts->layout[3] - ts->layout[2]) / 20);
		TP_PRINT("ts->key_postion_y: %d\n", ts->key_postion_y);
	}

	ret = synaptics_init_panel(ts);
	if (ret < 0) {
		pr_err("[TP] TOUCH_ERR: synaptics_init_panel fail\n");
	...
	if (ret) {
		pr_err("[TP] TOUCH_ERR: synaptics_ts_probe: "
	...
	if (rmi_char_dev_register())
		TP_PRINT(KERN_INFO "[TP] %s: error register char device", __func__);
#endif

	TP_PRINT(KERN_INFO "[TP] synaptics_ts_probe: Start touchscreen %s in %s mode\n", ts->input_dev->name, ts->use_irq ? "interrupt" : "polling");
	...
	if (ts->psensor_status == 0) {
		...
		if (ts->packrat_number < SYNAPTICS_FW_NOCAL_PACKRAT) {
			...
			if (ts->mfg_flag != 1) {
				...
				if (ts->energy_ratio_relaxation) {
					TP_PRINT(KERN_INFO "[TP] Enable ERR\n");
					ret = i2c_syn_write_byte_data(ts->client,
				...
				if (ts->saturation_bef_unlock) {
					...
					if (ret < 0)
						return i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "saturation capacitance", __func__);
					TP_PRINT(KERN_INFO "[TP] touch suspend, saturation capacitance: %x\n", ts->saturation_bef_unlock);
				}
				ret = i2c_syn_write_byte_data(ts->client,
					get_address_base(ts, 0x54, COMMAND_BASE), 0x04);
				if (ret < 0)
					i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "force update", __func__);
				TP_PRINT(KERN_INFO "[TP] touch suspend, fast relasxation: %x\n", ts->relaxation);
			}
#endif
			if (ts->large_obj_check) {
				if (ts->package_id == 2200) {
				...
				TP_PRINT(KERN_INFO "[TP] touch suspend, set large obj suppression: %x\n", ts->default_large_obj & 0x7F);
			}
			...
			if (ts->segmentation_bef_unlock) {
				...
				TP_PRINT(KERN_INFO "[TP] touch suspend, set segmentation aggressiveness: %x\n", ts->segmentation_bef_unlock);
			}
			...
			if (ts->threshold_bef_unlock) {
				...
				TP_PRINT(KERN_INFO "[TP] touch suspend, set Z Touch threshold: %x\n", ts->threshold_bef_unlock);
			}
		}
...
static int synaptics_ts_resume(struct i2c_client *client)
{
	...
	if (ts->power) {
		...
		if (ts->cable_support) {
			if (usb_get_connect_type())
				cable_tp_status_handler_func(1);
			TP_PRINT(KERN_INFO "%s: ts->cable_config: %x\n", __func__, ts->cable_config);
		}
#endif
	} else
#endif
	...
	if(ts->psensor_detection) {
		if(ts->psensor_status == 0) {
			...
			TP_PRINT(KERN_INFO "[TP] %s: Enable P-sensor by Touch\n", __func__);
