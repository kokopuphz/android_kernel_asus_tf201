...
static void AKECS_Report_Value(short *rbuf)
{
	...
	}else if (atomic_read(&a_flag) || atomic_read(&m_flag))
...
static int akm_aot_open(struct inode *inode, struct file *file)
{
	...
	if(atomic_cmpxchg(&open_count, 0, 1) == 0) {
		...
		D("[COMP] Compass akm_aot_release_assist first\n");
		akm_aot_release_assist();
		if (atomic_cmpxchg(&open_count, 0, 1) == 0) {
			atomic_set(&open_flag, 1);
			if (pdata->use_pana_gyro == 0)
				input_report_abs(data->input_dev, ABS_RUDDER, -1);
