	   ...
	   unsigned long arg)
{
	...
	case BMA_IOCTL_WRITE_CALI_VALUE:
		...
		D("%s: Write calibration value: 0x%X\n",
...
int bma250_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("BMA250 G-sensor I2C driver: gs_kvalue = 0x%X\n",
		pdata->gs_kvalue);
	...
	/*D("%s: CHIP ID = 0x%02x\n", __func__, buffer[0]);*/
	if (buffer[0] != 0x3) {
		pr_info("[GSNR] Use BMA250E solution chip_id is %x",buffer[0]);
		bma250_chip = 1;
	}
	else
		bma250_chip = 0;
