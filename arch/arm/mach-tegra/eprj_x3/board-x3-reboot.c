...
#define write_cmd_reserved_buffer(a,b)
#define read_cmd_reserved_buffer(a,b)
...
/* EternityProject MAX77663: LGE custom registers */
...
/* EternityProject MAX77663: END */
extern int max77663_set_ScratchRegister(u8 bit);
                                ...
                                unsigned long event, void *data)
{
		...
		/*
		 * EternityProject Note, 09/05/2013:
		 * RESERVE_0 is RESET
		 * RESERVE_1 is BOOTLOADER
		 * RESERVE_2 is UNLOCKED BOOTLOADER
		 */
            if (data) {
                printk("x3_reboot_notify : cmd = %s\n", (char *)data);
                if (!strcmp((char *)data, "bootloader")) {
                    printk("x3_reboot_notify: Rebooting to locked bootloader\n");
		          max77663_set_ScratchRegister(MAX77663_SCRATCH_REG_RESERVE_1);
                }
                else if (!strcmp((char *)data, "oem-unlock")) {
                    pr_info("x3_reboot_notify: Rebooting to unlocked bootloader\n");
                    max77663_set_ScratchRegister(MAX77663_SCRATCH_REG_RESERVE_2);
                }
            }
            return NOTIFY_OK;
         case SYS_HALT:
...
static ssize_t x3_rs_reset_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char rsbuf[3] = { 0,};
    ssize_t ret = 0;
    int rs = -1;
