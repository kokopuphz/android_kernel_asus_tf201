...
extern int forced_abort_hubevent; /*EPRJ-LGE*/
static int xmm_power_on(struct platform_device *device)
{
	...
	if (!modem_flash) {
		...
		forced_abort_hubevent = 0; /* EPRJ-LGE */
	...
	if (ret < 0)
		pr_err("%s:EPRJ: can't set irq wake: error!\n", __func__);
	pr_debug("%s }\n", __func__);
...
/*EPRJ	HACK*/ bool enum_success = true;
...
//		disable_irq(gpio_to_irq(pdata->modem.xmm.ipc_ap_wake)); //EPRJ
	...
	enable_irq(gpio_to_irq(pdata->modem.xmm.ipc_ap_wake));
